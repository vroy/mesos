...
TEST_F(MasterQuotaTest, ChildRole)
{
  ...
  // Query the configured quota.
  {
    ...
    EXPECT_NE(status->infos(0).role(), status->infos(1).role());

    // Don't assume that the quota for child and parent are returned
    // in any particular order.
    for (int i = 0; i < status->infos().size(); i++) {
      const QuotaInfo& info = status->infos(i);

      EXPECT_TRUE(info.role() == PARENT_ROLE ||
                  info.role() == CHILD_ROLE);

      if (info.role() == PARENT_ROLE) {
        EXPECT_EQ(parentQuotaResources, info.guarantee());
      } else {
        EXPECT_EQ(childQuotaResources, info.guarantee());
      }
    }
  }
...
TEST_F(MasterQuotaTest, ChildRoleSumExceedsParentQuota)
{
  ...
  // Attempt to set quota for the first child role. This should fail,
