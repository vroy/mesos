...
public:
      ...
      const std::string& sandboxDirectory)
  {
    ...
    // Copy the global rotation settings.
    // These will act as the defaults in case the executor environment
    // overrides a subset of them.
    LoggerOptions options;
    options.max_stdout_size = flags.max_stdout_size;
    options.logrotate_stdout_options = flags.logrotate_stdout_options;
    options.max_stderr_size = flags.max_stderr_size;
    options.logrotate_stderr_options = flags.logrotate_stderr_options;
    ...
    if (executorInfo.has_command() &&
      ...
      Try<flags::Warnings> load = options.load(executorEnvironment);

      if (load.isError()) {
        return Failure(
            "Failed to load executor logger options: " + load.error());
      }

      // We don't expect any warnings, so fail if there are any.
      if (load->warnings.size() > 0) {
        return Failure(
            "Unexpected warning while loading executor logger options: "
            + load->warnings[0].message);
      }
    }
    ...
    outFlags.max_size = options.max_stdout_size;
    outFlags.logrotate_options = options.logrotate_stdout_options;
    ...
    errFlags.max_size = options.max_stderr_size;
    errFlags.logrotate_options = options.logrotate_stderr_options;
